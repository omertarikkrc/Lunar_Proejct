...
v_r       = mu_moon/h_orb * e * sin(phi_imp);
...
% --- 3) İniş yolunu çiz (Ay yüzeyi, eliptik yörünge, iniş trajektoryası) ---
...
% 3.2) Eliptik yörünge (tüm true anomaly 0→2π)
theta_e = linspace(0,2*pi,500);
r_e     = p ./ (1 + e*cos(theta_e));    % p = a*(1-e^2) zaten hesaplı
x_ell   =  r_e .* sin(theta_e);
y_ell   = -r_e .* cos(theta_e);
plot(x_ell, y_ell, 'm--', 'LineWidth',1);

% 3.3) Spacecraft iniş trajektoryası
plot(Y(:,1), Y(:,2), 'b-', 'LineWidth',1.5);

% 3.4) İnişe başladığı nokta (elliptik perigee)
plot(x_imp, y_imp, 'ro', 'MarkerSize',8, 'LineWidth',1.5);

xlabel('x [m]'); 
ylabel('y [m]'); 
title('Phase 2 İnişi: Ay Yüzeyi, Eliptik Yörünge ve Descent');
legend('Ay yüzeyi','Eliptik yörünge','İniş trajektoryası','İtki başlangıcı', ...
       'Location','Best');
